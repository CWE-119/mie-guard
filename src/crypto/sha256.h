/*
 * Minimal single-header SHA-256 implementation.
...
static inline void sha256_update(sha256_ctx_t *ctx,
...
static inline void sha256_final(sha256_ctx_t *ctx, uint8_t out[32])
{
    ...
    sha256_transform(ctx, ctx->buf);
    for (int j=0; j<8; j++) {
        out[j*4  ] = (ctx->state[j]>>24)&0xFF;
        out[j*4+1] = (ctx->state[j]>>16)&0xFF;
        out[j*4+2] = (ctx->state[j]>> 8)&0xFF;
        out[j*4+3] = (ctx->state[j]    )&0xFF;
    }
}
